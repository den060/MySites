<div id="breadcrumbs">
    <ul>
        ...
        <li><span class="arrow"> • </span></li>
        <li><span class="changeName">Поиск товара</span></li>
    </ul>
</div>
...
<div id="catalog">
    ...
    <?php $store = $pdo->prepare("SELECT * FROM items WHERE name LIKE :name limit 25"); $store->execute(['name' => "%".$_GET["q"]."%"]); while($row = $store->fetch()): ?>
    ...
    <div class="itemRow item sku">
        ...
        <?php endwhile; ?>
            ...
            <?php $store = $pdo->prepare("SELECT * FROM items WHERE brand LIKE :brand limit 25"); $store->execute(['brand' => $_GET["q"]]); while($row = $store->fetch()): ?>
